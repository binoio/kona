//
//  EditWakeStateView.swift
//  Kona
//
//  Created by GitHub Copilot on 2025-12-26.
//

import SwiftUI

struct EditWakeStateView: View {
    @ObservedObject var state: WakeState
    @EnvironmentObject var manager: WakeStateManager

    @State private var allowScreenDim: Bool
    @State private var allowSystemLock: Bool
    @State private var sleepAtEnd: Bool
    @State private var selectedDays: Set<Weekday> = []
    @State private var startTime: Date = Date()
    @State private var endTime: Date = Date().addingTimeInterval(3600)

    init(state: WakeState) {
        self.state = state
        _allowScreenDim = State(initialValue: state.options.allowScreenDim)
        _allowSystemLock = State(initialValue: state.options.allowSystemLock)
        _sleepAtEnd = State(initialValue: state.options.sleepAtEnd)
        if let schedule = state.schedule {
            _selectedDays = State(initialValue: Set(schedule.days))
            _startTime = State(initialValue: schedule.startTime)
            _endTime = State(initialValue: schedule.endTime)
        }
    }

    var body: some View {
        Form {
            Section {
                Text(state.name)
                    .font(.title3.weight(.semibold))
            }
            Toggle("Allow screen dim", isOn: $allowScreenDim)
                .onChange(of: allowScreenDim) { _ in save() }
            Toggle("Allow system lock", isOn: $allowSystemLock)
                .onChange(of: allowSystemLock) { _ in save() }
            if state.duration.timeInterval != nil {
                Toggle("Sleep at the end", isOn: $sleepAtEnd)
                    .onChange(of: sleepAtEnd) { _ in save() }
            }

            if state.duration == .scheduled {
                Divider()
                    .padding(.vertical, 8)

                DayPicker(selections: $selectedDays)
                    .onChange(of: selectedDays) { _ in save() }
                DatePicker("Start time", selection: $startTime, displayedComponents: .hourAndMinute)
                    .onChange(of: startTime) { _ in save() }
                DatePicker("End time", selection: $endTime, displayedComponents: .hourAndMinute)
                    .onChange(of: endTime) { _ in save() }
            }
        }
        .formStyle(.grouped)
        .padding()
    }

    private func save() {
        state.options = WakeState.StateOptions(allowScreenDim: allowScreenDim, allowSystemLock: allowSystemLock, sleepAtEnd: sleepAtEnd)
        if state.duration == .scheduled {
            state.schedule = WakeState.Schedule(days: Array(selectedDays), startTime: startTime, endTime: endTime)
        } else {
            state.schedule = nil
        }
        manager.saveWakeStates()
    }
}

struct DayPicker: View {
    @Binding var selections: Set<Weekday>

    var body: some View {
        HStack(spacing: 4) {
            ForEach(Weekday.allCases, id: \.self) { day in
                Button(action: {
                    if selections.contains(day) {
                        selections.remove(day)
                    } else {
                        selections.insert(day)
                    }
                }) {
                    Text(day.shortName)
                        .font(.caption)
                        .fontWeight(selections.contains(day) ? .semibold : .regular)
                        .frame(width: 28, height: 28)
                        .background(selections.contains(day) ? Color.accentColor : Color.secondary.opacity(0.2))
                        .foregroundColor(selections.contains(day) ? .white : .primary)
                        .clipShape(Circle())
                }
                .buttonStyle(.plain)
            }
        }
    }
}

struct MultiSelectPicker<T: Hashable & CaseIterable & RawRepresentable>: View where T.RawValue == String {
    let title: String
    @Binding var selections: Set<T>
    let options: [T]

    var body: some View {
        List {
            ForEach(options, id: \.self) { option in
                HStack {
                    Text(option.rawValue.capitalized)
                    Spacer()
                    if selections.contains(option) {
                        Image(systemName: "checkmark")
                    }
                }
                .onTapGesture {
                    if selections.contains(option) {
                        selections.remove(option)
                    } else {
                        selections.insert(option)
                    }
                }
            }
        }
        .navigationTitle(title)
    }
}
